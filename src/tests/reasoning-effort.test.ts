...
describe("Reasoning Effort & Model Capability Detection", () => {
  ...
  it("has complete reasoning effort options definitions (minimal, low, medium, high, max)", () => {
    expect(REASONING_EFFORT_OPTIONS).toHaveLength(5);
    const ids = REASONING_EFFORT_OPTIONS.map((o) => o.id);
    expect(ids).toEqual(["minimal", "low", "medium", "high", "max"]);
